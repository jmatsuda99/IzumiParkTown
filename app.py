...
class IzumiPowerAnalyzer:
    ...
    def _build_ui(self):
        ...
        nav_group = ttk.LabelFrame(top_row, text="移動", padding=8)
        nav_group.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        if self.on_back is not None:
            ttk.Button(nav_group, text="ツール選択へ戻る", command=self.on_back).pack(side="left")
        else:
            ttk.Label(nav_group, text="この画面が起点です").pack(side="left")

        data_group = ttk.LabelFrame(top_row, text="データ読込", padding=8)
        data_group.grid(row=0, column=1, sticky="ew", padx=4)
        ttk.Button(data_group, text="Excel/ZIP読込→DB保存", command=self.load_file).pack(side="left", padx=3)
        ttk.Button(data_group, text="DB一覧", command=self.show_db_list).pack(side="left", padx=3)
        ttk.Button(data_group, text="最新DBを読込", command=self.load_latest_from_db).pack(side="left", padx=3)

        pv_group = ttk.LabelFrame(top_row, text="PV設定", padding=8)
        pv_group.grid(row=0, column=2, sticky="ew", padx=(8, 0))
        ttk.Button(pv_group, text="PV正規化CSV/ZIP読込", command=self.load_pv_profile_file).pack(side="left", padx=3)
        ttk.Checkbutton(pv_group, text="PV反映", variable=self.use_pv_var).pack(side="left", padx=(6, 2))
        ttk.Label(pv_group, text="PV係数:").pack(side="left", padx=(6, 2))
        self.pv_factor_entry = ttk.Entry(pv_group, textvariable=self.pv_factor_var, width=8)
        self.pv_factor_entry.pack(side="left")

        bottom_row = ttk.Frame(toolbar)
        bottom_row.grid(row=1, column=0, columnspan=3, sticky="ew", pady=(8, 0))
        bottom_row.columnconfigure(0, weight=1)
        bottom_row.columnconfigure(1, weight=1)
        bottom_row.columnconfigure(2, weight=1)
        bottom_row.columnconfigure(3, weight=1)

        analysis_group = ttk.LabelFrame(bottom_row, text="分析表示", padding=8)
        analysis_group.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        ttk.Button(analysis_group, text="年間概要", command=self.show_summary).pack(side="left", padx=3)
        ttk.Button(analysis_group, text="日別負荷", command=self.show_daily_profile).pack(side="left", padx=3)
        ttk.Button(analysis_group, text="月別集計", command=self.show_monthly_usage).pack(side="left", padx=3)
        ttk.Button(analysis_group, text="ヒートマップ", command=self.show_heatmap).pack(side="left", padx=3)
        ttk.Button(analysis_group, text="月別 平日/休祝日平均", command=self.show_monthly_weekday_holiday).pack(
            side="left", padx=3
        )

        day_group = ttk.LabelFrame(bottom_row, text="指定日", padding=8)
        day_group.grid(row=0, column=1, sticky="ew", padx=4)
        ttk.Label(day_group, text="日付(YYYY-MM-DD):").pack(side="left", padx=(0, 4))
        self.date_entry = ttk.Entry(day_group, width=14)
        self.date_entry.pack(side="left")
        ttk.Button(day_group, text="指定日グラフ", command=self.show_selected_day).pack(side="left", padx=4)

        export_group = ttk.LabelFrame(bottom_row, text="出力", padding=8)
        export_group.grid(row=0, column=2, sticky="ew", padx=4)
        ttk.Button(export_group, text="CSV出力", command=self.export_csv).pack(side="left")

        status_group = ttk.LabelFrame(bottom_row, text="状態", padding=8)
        status_group.grid(row=0, column=3, sticky="ew", padx=(8, 0))
        ttk.Label(status_group, textvariable=self.status_var).pack(anchor="w")
