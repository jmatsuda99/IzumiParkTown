...
class IzumiPowerAnalyzer:
    ...
    def show_summary(self):
        ...
        avg_30m = self.df["kwh"].mean()
        max_30m = self.df["kwh"].max()
        min_30m = self.df["kwh"].min()
        ...
        self.log(f"30分平均: {avg_30m:,.2f} kWh")
        self.log(f"30分最大: {max_30m:,.2f} kWh")
        self.log(f"30分最小: {min_30m:,.2f} kWh")
        self.log(f"日平均: {avg_day:,.2f} kWh/日")
        self.log(f"日最大: {max_day:,.2f} kWh/日")
        self.log(f"日最小: {min_day:,.2f} kWh/日")

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.plot(self.daily_df["date"], self.daily_df["kwh"])
        ax.set_title("日別使用電力量")
        ax.set_xlabel("Date")
    ...
    def show_daily_profile(self):
        ...
        for _, row in profile.head(10).iterrows():
            self.log(f"{row['time']} : {row['kwh']:.2f} kWh")

        fig, ax = plt.subplots(figsize=(11, 5))
        ax.plot(profile["time"], profile["kwh"])
        ax.set_title("平均日負荷カーブ（30分値平均）")
        ax.set_xlabel("Time")
    ...
    def show_heatmap(self):
        ...
        pivot = work.pivot_table(index="time", columns="date_str", values="kwh", aggfunc="mean")
    ...
    def export_csv(self):
        ...
        self.daily_df.to_csv(out_dir / "izumi_daily_totals.csv", index=False, encoding="utf-8-sig")
