...
class IzumiPowerAnalyzer:
    ...
    def _build_ui(self):
        ...
        self.text = tk.Text(left, wrap="word", width=52)
    ...
    def prepare_views(self):
        self.df["date_ts"] = pd.to_datetime(self.df["datetime"]).dt.normalize()
        self.df["date"] = pd.to_datetime(self.df["datetime"]).dt.date
        self.df["month"] = pd.to_datetime(self.df["datetime"]).dt.to_period("M").astype(str)
        self.df["time"] = pd.to_datetime(self.df["datetime"]).dt.strftime("%H:%M")
    ...
    def show_summary(self):
        ...
        avg_30m_kw = self.df["kwh"].mean() / 0.5
        max_30m_kw = self.df["kwh"].max() / 0.5
        min_30m_kw = self.df["kwh"].min() / 0.5
